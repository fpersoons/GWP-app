...
Application d'évaluation des Bonnes Pratiques d'Entreposage (OMS TRS 1025) pour le projet GHSC-PSM / Chemonics. Voir [README.md](README.md) pour les fonctionnalités, le lancement local et le déploiement (Netlify, site `gwp-app`, à chaque push sur `main` ; exposé sur https://www.chemlink.app/GWP-app/ via le portail).
...
- Icônes : SVG inline au trait (style Lucide, `stroke-width 2.5`), jamais d'emoji ni de police d'icônes — bibliothèque dans `app/js/icons.js` (`icon(nom, taille)`).
- Variables de rôle (`--primary`, `--positive`, `--warning`, `--negative`, `--label`…) et palette `--chem-*` en tête de `app/css/app.css` ; paliers et couleurs de domaines dans `app/data/criteria.js`.
